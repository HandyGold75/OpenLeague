...
func _ready() -> void:
	...
	if len(connected_controllers) <= 0:
		...
		ConfigHandler.hotconfig.selected_controller = null
	elif ConfigHandler.hotconfig.selected_controller == null:
		controller_option_button.select(1)
		ConfigHandler.hotconfig.selected_controller = connected_controllers[0]
...
func _on_controller_item_selected(index: int) -> void:
	if index == 0:
		ConfigHandler.hotconfig.selected_controller = null
	ConfigHandler.hotconfig.selected_controller = connected_controllers[index - 1]


func _on_steer_sens_value_changed(value: float) -> void:
	ConfigHandler.save_control_config("steer_sens", int(value))


func _on_tilt_sens_value_changed(value: float) -> void:
	ConfigHandler.save_control_config("tilt_sens", int(value))


func _on_joystick_deadzone_value_changed(value: float) -> void:
	ConfigHandler.save_control_config("joystick_deadzone", float(value))


func _on_cam_rotation_sens_value_changed(value: float) -> void:
	ConfigHandler.save_control_config("camera_rotation_sens", int(value))


func _on_cam_center_speed_value_changed(value: float) -> void:
	ConfigHandler.save_control_config("camera_center_speed", int(value))


func _on_cam_center_delay_value_changed(value: float) -> void:
